...
def obtainSwitchData():
    ...
    subprocess.call('sudo ifconfig en0 hw ether aa:bb:bb:bb:bb:bb',shell=True)
...
def getEth0Status():
    wiredConnection = subprocess.check_output('ifconfig en0 | grep "status"',shell=True)
    print wiredConnection
    wiredConnection = wiredConnection.split(' ')[1]
    print wiredConnection
    wiredConnection = wiredConnection.strip()
    print wiredConnection
    if (str(wiredConnection)=="active"):
        return True
    else:
...
while True:
    if (getEth0Status() == True):
        subprocess.call('sudo ifconfig en0 down',shell=True)
        subprocess.call('sudo ifconfig en0 hw ether 12:aa:aa:aa:aa:aa',shell=True)
        subprocess.call('sudo ifconfig en0 up',shell=True)
        x=0
        while(x<2):
            if(obtainSwitchData()):
